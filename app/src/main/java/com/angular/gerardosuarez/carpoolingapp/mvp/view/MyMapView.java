...
public class MyMapView extends FragmentView<MyMapFragment, Void> {
    ...
    Button btnDate;
    @BindView(R.id.btn_cancel_route)
    Button btnCancel;
    ...
    private void setNotChoosedRedMarker(LatLng latLng, String title, int id) {
        ...
        map.addCircle(new CircleOptions()
                .center(latLng)
                .radius(100)
                .strokeColor(getActivity().getResources().getColor(R.color.area))
                .fillColor(getActivity().getResources().getColor(R.color.area)));
    }
    ...
    private void setChoosedByDriverBlueMarker(LatLng latLng, String title, int id) {
        if (getContext() == null) return;
        Bitmap scaledBitmap = getScaledBitmap(R.drawable.blue_marker, getContext().getResources());
        BitmapDescriptor icon = BitmapDescriptorFactory.fromBitmap(scaledBitmap);
        scaledBitmap.recycle();
        map.addMarker(new MarkerOptions()
                .position(latLng)
    ...
    public void addMarkerForChoosenPassengerThatIsNotMine(PassengerBooking passengerBooking, int id) {
        setChoosedByDriverBlueMarker(new LatLng(passengerBooking.latitude, passengerBooking.longitude), passengerBooking.getKey(), id);
    }
    ...
    public void showDialogQuota(DisposableObserver<PassengerBooking> observer, PassengerBooking passengerBooking,String com,String fot) {
        if (getActivity() == null) return;
        dialogMarkerClick = new DialogMarker(getActivity(), passengerBooking,com,fot);
    ...
    public void removeDialog(){
        if(dialogMarkerClick==null) return;
        dialogMarkerClick.dismiss();
    }

    public void hideBtnCancel(boolean doHide,String rol){
        if(doHide){
            btnCancel.setVisibility(View.GONE);
        }else {
            if(rol.equals("driver")) {
    ...
    public void setTextQuota(String cupo){
        textQuota.setText(getFragment().getText(R.string.current_quota) + cupo);
    }
    public void appendTextQuota(String cupo){
        textQuota.append(cupo);
    }

    public void hideTextQuota(boolean hide){
        if(hide){
